<?
	...
	if (IsSet($_POST["action"])) {
		if ($_POST["action"] == "save") {
			...
			if (IsSet($_POST['id'])) {
				...
				if ($can_save) {
					$str_query ="
						UPDATE account_pt_status
						SET status = '".$_POST["status"]."'
						WHERE (status_id = $int_id)";
	...
	if (IsSet($_GET["id"])) {
		...
		");
		if ($qry->RowCount() == 0) {
			$str_message = "ERROR: Status not found.";
		}
		
		$str_status = $qry->FieldByName('status');
	}
...
<? if ($str_message != '') echo "<font color=\"red\">".$str_message."</font>";?>
	...
	<table width="100%" height="30" border="0" cellpadding="2" cellspacing="0">
		<tr>
			<td class='normaltext' align="right" width="120">Status</td>
			<td><input type="text" name="status" class='input_200' value="<? echo $str_status;?>" autocomplete="OFF" onkeypress="focusNext(this, 'account_number', event)"></td>
		</tr>
		<tr>
			<td align="right">
...
<script language="javascript">
	document.account_edit.status.focus();
</script>
