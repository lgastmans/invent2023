	...
	<head>
		...
		<script language="javascript">
			/*
				CUSTOM FIELDS
			*/
			var SQL = "SELECT ac.account_id, ac.account_name, ac.account_number, ac.account_status, "+
				"ac.enabled, c.community_name, ab.opening_balance, ab.closing_balance "+
				"FROM account_pt ac LEFT JOIN communities c ON (ac.community_id = c.community_id) "+
				"LEFT JOIN <? echo Monthalize('account_pt_balances'); ?> ab ON (ac.account_id = ab.account_id)";
			var ID = "account_id";
//			var uniqueFilter = "b.storeroom_id = <?php echo $_SESSION['int_current_storeroom']?>";
			...
			function newRecord() {
				myWin = window.open("account_edit.php",'insert','toolbar=no,fullscreen=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=450,height=550,top=0,left=0');
			...
			function editRecord() {
				if (selectedID > 0) {
					myWin = window.open("account_edit.php?id="+selectedID, 'edit', 'width=450,height=550,resizable=yes');
					myWin.moveTo((screen.availWidth/2 - 450/2), (screen.availHeight/2 - 550/2));
					myWin.focus();
				}
				else
			...
			<?php
				if (($_SESSION["int_month_loaded"] == date('n')) && ($_SESSION["int_year_loaded"] == date('Y'))) {
					if ($int_access_level > ACCESS_READ) {
						echo "&nbsp;<a href=\"javascript:newRecord();\"><img src=\"../images/page_white_add.png\" border=\"0\" title=\"New account\"></a>\n";
						echo "&nbsp;<a href=\"javascript:editRecord();\"><img src=\"../images/page_white_edit.png\" border=\"0\" title=\"Edit the details of the selected account\"></a>\n";
					}
				}
			?>
			&nbsp;
	...
	YAHOO.util.Event.addListener(window, "load", function() {
		myGrid = function() {
						...
						if (canDelete) {
							if (confirm('Are you sure you want to delete this row?')) {
								...
								YAHOO.util.Connect.asyncRequest(
									'GET',
									'account_delete.php?action=delete&id='+selectedID,
									{
										success: function (o) {
			...
			var onCellDblClick = function(oArgs) {
				...
				myWin = window.open("account_edit.php?action=view&id="+selectedID,'bills_grid_details','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=800,height=600');
