<?
	...
	$grid = new DBGrid('pt_accounts_nationality');
	
	$grid->addColumn("Nationality", "nationality", "string", true, 200);
	
	$grid->loadView();
	
	$grid->setQuery("
		SELECT *
		FROM account_pt_nationality apn
	");
	
	$grid->setOnClick('gridClick','nationality_id');
	$grid->setSubmitURL('nationality.php');
	$grid->processParameters($_GET);
	$grid->addOrder('nationality', 'ASC');
...
function newRecord() {
	myWin = window.open("nationality_edit.php",'account_new','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=450,height=550,top=0');
...
function modifyRecord() {
  ...
  if (sNo>0) {
    myWin = window.open("nationality_edit.php?id="+intSerialNumber,'account_edit','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=450,height=550,top=0');
	myWin.moveTo((screen.availWidth/2 - 450/2), (screen.availHeight/2 - 550/2));
    myWin.focus();
  }
  else {
