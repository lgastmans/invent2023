<?
	...
	/*
		CUSTOM FIELDS
	*/
	$_SESSION["int_pt_accounts_selected"] = 6;
	
	$grid_name = 'ptaccounts_nationality';
	$default_alias = 'apn';
...
/*			.ff th.yui-dt-hidden,
		...
		<script language="javascript">
			/*
				CUSTOM FIELDS
			*/
			var SQL = "SELECT * FROM account_pt_nationality apn";
			var ID = "nationality_id";
			...
			function editRecord() {
				if (selectedID > 0) {
					myWin = window.open("nationality_edit.php?id="+selectedID, 'edit', 'width=800,height=700,resizable=yes');
			...
			function newRecord() {
				myWin = window.open("nationality_edit.php",'insert','toolbar=no,fullscreen=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=800,height=700,top=0,left=0');
				myWin.moveTo((screen.availWidth/2 - 800/2), (screen.availHeight/2 - 700/2));
				myWin.focus();
			}
...
/*		var myBody = document.getElementById('myBody');
		...
		myGrid = function() {
						...
						if (canDelete) {
							if (confirm('Are you sure you want to delete this row?')) {
								...
								YAHOO.util.Connect.asyncRequest(
									'GET',
									'nationality_delete.php?action=delete&id='+selectedID,
									{
										success: function (o) {
			...
			var onCellDblClick = function(oArgs) {
				...
				myWin = window.open("nationality_edit.php?action=view&id="+selectedID,'tax_grid_details','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=650,height=550');
