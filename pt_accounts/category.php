<?
	...
	$grid = new DBGrid('pt_accounts_category');
	
	$grid->addColumn("Category", "category", "string", true, 200);
	$grid->addColumn("Description", "description", "string", true, 200);
	$grid->addColumn("Amount", "amount", "number", true, 100);
	...
	$grid->setOnClick('gridClick','category_id');
	$grid->setSubmitURL('category.php');
	$grid->processParameters($_GET);
	$grid->addOrder('category', 'ASC');
	...
	$_SESSION["int_pt_accounts_selected"] = 5;
...
function newRecord() {
	myWin = window.open("category_edit.php",'account_new','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=450,height=550,top=0');
...
function modifyRecord() {
  ...
  if (sNo>0) {
    myWin = window.open("category_edit.php?id="+intSerialNumber,'account_edit','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=450,height=550,top=0');
	myWin.moveTo((screen.availWidth/2 - 450/2), (screen.availHeight/2 - 550/2));
    myWin.focus();
  }
  else {
