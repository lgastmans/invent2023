...
/*			.ff th.yui-dt-hidden,
		...
		<script language="javascript">
			/*
				CUSTOM FIELDS
			*/
			var SQL = "SELECT * FROM account_pt_status aps";
			var ID = "status_id";
			...
			function editRecord() {
				if (selectedID > 0) {
					myWin = window.open("status_edit.php?id="+selectedID, 'edit', 'width=800,height=700,resizable=yes');
			...
			function newRecord() {
				myWin = window.open("status_edit.php",'insert','toolbar=no,fullscreen=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=800,height=700,top=0,left=0');
				myWin.moveTo((screen.availWidth/2 - 800/2), (screen.availHeight/2 - 700/2));
				myWin.focus();
			}
	...
	<tr>
		<td><?
			if ($int_access_level > ACCESS_READ) {
				echo "&nbsp;<a href=\"javascript:newRecord();\"><img src=\"../images/page_white_add.png\" border=\"0\" title=\"add a new status\"></a>\n";
				echo "&nbsp;<a href=\"javascript:editRecord();\"><img src=\"../images/page_white_edit.png\" border=\"0\" title=\"edit the details of the selected status\"></a>\n";
			}
		?>
			&nbsp;
...
/*		var myBody = document.getElementById('myBody');
		...
		myGrid = function() {
						...
						if (canDelete) {
							if (confirm('Are you sure you want to delete this row?')) {
								...
								YAHOO.util.Connect.asyncRequest(
									'GET',
									'status_delete.php?action=delete&id='+selectedID,
									{
										success: function (o) {
			...
			var onCellDblClick = function(oArgs) {
				...
				myWin = window.open("status_edit.php?action=view&id="+selectedID,'tax_grid_details','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=650,height=550');
