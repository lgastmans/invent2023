<?
	...
	if (IsSet($_POST["action"])) {
		if ($_POST["action"] == "save") {
			...
			if (IsSet($_POST['id'])) {
				...
				if ($can_save) {
					$str_query ="
						INSERT INTO account_pt_nationality
						(
							nationality
						)
						VALUES (
	...
	if (IsSet($_GET["id"])) {
		...
		");
		if ($qry->RowCount() == 0) {
			$str_message = "ERROR: Nationality not found.";
		}
		
		$str_nationality = $qry->FieldByName('nationality');
	}
...
<? if ($str_message != '') echo "<font color=\"red\">".$str_message."</font>";?>
	...
	<table width="100%" height="30" border="0" cellpadding="2" cellspacing="0">
		<tr>
			<td class='normaltext' align="right" width="120">Nationality</td>
			<td><input type="text" name="nationality" class='input_200' value="<? echo $str_nationality;?>" autocomplete="OFF" onkeypress="focusNext(this, 'account_number', event)"></td>
		</tr>
		<tr>
			<td align="right">
...
<script language="javascript">
	document.account_edit.nationality.focus();
</script>
