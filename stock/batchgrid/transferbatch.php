...
function saveForm($values) {
   global $msg;
	...
	$qry_exists->Query("
    	...
    	if ($qry_exists->FieldByName("batch_id") <> $values['id']) {
...
//	echo "Updating Storeroom Batch";
	$msg = updateStoreroomBatch($_SESSION['int_current_storeroom'],$values['id'],0,$values['transfer_quantity'],0);

	$msg .= updateStoreroomProduct($_SESSION['int_current_storeroom'],$values['product_id'],0,$values['transfer_quantity'],0);	
	$msg.=updateStoreroomBatch($values['storeroom_id'],$values['id'],0,0,$values['transfer_quantity']);
...
//	echo $str_update;
	$qry_exists->ExecuteQuery($str_update);
