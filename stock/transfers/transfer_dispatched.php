...
/**
* 
* @version 	$Id: transfer_dispatched.php,v 1.2 2006/02/20 03:58:37 cvs Exp $
...
*/
  ...
  $_SESSION['str_context_help']='stock/help/transfer_dispatched.html';
...
$grid = new DBGrid('stock_dispatched');
...
$grid->addColumn("Req. By", "u.username", "string", true); 
$grid->addColumn("Disp. By", "ud.username", "string", true); 
$grid->loadView();
...
$grid->setQuery("SELECT
	...
	udel.username as `udel.username`,
...
LEFT JOIN
	user udel on udel.user_id = ssb.user_id_received
...
$grid->setSubmitURL('transfer_dispatched.php');
...
$grid->addUniqueFilter('ssb.transfer_status','equals',STATUS_DISPATCHED,'number');
$grid->addUniqueFilter('sp.deleted','equals','N','string');
$grid->processParameters($_GET);
if ($grid->bool_show_deleted) 
	$grid->addColumn("Deleted By", "udel.username", "string", true); 
...
if (!empty($_GET["action"])) {
  if ($_GET["action"]=="del") {
    require("stockcomplete.inc.php");
    
    $_SESSION['str_stock_message'] = deleteRecord($_GET["delid"]);
    
    header("Location:transfer_dispatched.php?".$grid->buildQueryString());
    exit;
  }
  if ($_GET["action"]=="complete") {
    require("stockcomplete.inc.php");
    
    $_SESSION['str_stock_message'] = completeRecord($_GET["delid"]);
...
function transferRecord() {
  ...
  if (sNo>0) {
  myWin = window.open("transfertransfer_dispatched.php",'stock','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=450,height=550,top=0');
  myWin.focus();
...
function dispatchTransfer() {
  ...
  if (sNo>0) {
        if (document.location.href.indexOf("?")<0) {
          document.location = document.location.href+"?action=complete&delid="+sNo;
	} else {
	  document.location = document.location.href+"&action=complete&delid="+sNo;
	}
...
if (($_SESSION["int_month_loaded"] == date('n')) && ($_SESSION["int_year_loaded"] == date('Y'))) {
  if ($int_access_level > ACCESS_READ) {
  	$grid_form->addButton('Complete Selected Transfer (Receiving party only)','../../images/export.gif','dispatchTransfer()','left');
  	$grid_form->addButton('Cancel Selected Transfer (Sending party only)','../../images/delete.gif','deleteTransfer()','left');
