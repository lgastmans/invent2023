...
/*			th.yui-dt-hidden,
		...
		<script language="javascript">
			/*
				CUSTOM FIELDS
			*/
			var SQL = "SELECT rts_item_id, quantity, price, product_code, product_description, batch_code, measurement_unit, is_decimal "+
				"FROM <?php echo Monthalize('stock_rts_items');?> sri "+
				"INNER JOIN stock_product sp ON (sp.product_id = sri.product_id) "+
				"INNER JOIN <?php echo Yearalize('stock_batch');?> sb ON (sb.batch_id = sri.batch_id) "+
        			"INNER JOIN stock_measurement_unit mu ON sp.measurement_unit_id=mu.measurement_unit_id";
			var ID = "rts_item_id";
			var uniqueFilter = "sri.rts_id = <?php echo $_GET['id']?>";
