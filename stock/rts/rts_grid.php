...
/*			.ff th.yui-dt-hidden,
		...
		<script language="javascript">
			/*
				CUSTOM FIELDS
			*/
			var SQL = "SELECT stock_rts_id, storeroom_id, bill_number, date_created, total_amount, discount, description, bill_status, "+
				"username, supplier_name "+
				"FROM <?php echo Monthalize('stock_rts');?> sr "+
				"INNER JOIN user u ON (u.user_id = sr.user_id) "+
				"INNER JOIN stock_supplier ss ON (sr.supplier_id = ss.supplier_id)";
			var ID = "stock_rts_id";
			var uniqueFilter = "sr.storeroom_id = <?php echo $_SESSION['int_current_storeroom']?>";
			...
			function newReceipt() {
				myWin = window.open("receipt_frameset.php?action=clear_receipt",'create_receipt','toolbar=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=800,height=600,top=0');
			...
			function editReceipt() {
				if (selectedID > 0) {
					myWin = window.open("receipt_frameset.php?id="+selectedID,'edit_receipt','toolbar=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=800,height=600,top=0');
					myWin.moveTo((screen.availWidth/2 - 800/2), (screen.availHeight/2 - 600/2));
					myWin.focus();
				}
				else
					alert("Select a receipt to edit");
			}

			function exportNote() {
				if (selectedID > 0) {
					myWin = window.open("../export_debit_note.php?id="+selectedID, 'genDebitNote', 'width=800,height=500,resizable=yes,menubar=yes'); 
				}
				else
					alert('Select a debit note');
			}


			function exportCSV() {
				if (selectedID > 0) {
					myWin = window.open("export_dn_csv.php?id="+selectedID, 'export_dn_csv', 'width=800,height=500,resizable=yes,menubar=yes'); 
				}
				else
					alert('Select a debit note');
			}
			...
			function printReceipt() {
				if (selectedID > 0) {
					myWin = window.open("print_receipt.php?id="+selectedID, 'printwin', 'width=800,height=500,resizable=yes');
				}
				else
			...
			<?php
				if (($_SESSION["int_month_loaded"] == date('n')) && ($_SESSION["int_year_loaded"] == date('Y'))) {
					if ($int_access_level > ACCESS_READ) {
						echo "&nbsp;<a href=\"javascript:newReceipt();\"><img src=\"../../images/page_white_add.png\" border=\"0\" title=\"New receipt\"></a>\n";
						echo "&nbsp;<a href=\"javascript:editReceipt();\"><img src=\"../../images/page_white_edit.png\" border=\"0\" title=\"Edit receipt\"></a>\n";
						echo "&nbsp;<a href=\"javascript:cancelReceipt();\"><img src=\"../../images/cancel.png\" border=\"0\" title=\"Cancel selected receipt\"></a>\n";
					}
				}
			?>

			&nbsp;
			<a href="javascript:exportNote()"><img src="../../images/pdf-icon.png" border="0" title="Generate Debit Note"></a>

			&nbsp;
			<a href="javascript:printReceipt();"><img src="../../images/printer.png" border="0" title="print the selected receipt"></a>
			&nbsp;
			<a href="javascript:printGrid('Y');"><img src="../../images/printer.png" border="0" title="print the content of the grid"></a>
			&nbsp;
			<a href="javascript:exportCSV();"><img src="../../images/csv_export.png" border="0" title="export the content to a CSV file (tab delimited)"></a>
...
/*		var myBody = document.getElementById('myBody');
		...
		myGrid = function() {
			...
			parseBillStatus = function(oData) {
				switch (oData) {
					case '<?php echo BILL_STATUS_UNRESOLVED;?>': return "Unresolved";break;
					case '<?php echo BILL_STATUS_RESOLVED;?>': return "Resolved";break;
					case '<?php echo BILL_STATUS_CANCELLED;?>': return "<font color='red'>Cancelled</font>";break;
			...
			var onCellDblClick = function(oArgs) {
				...
				myWin = window.open("rts_grid_details.php?action=view&id="+selectedID,'rts_grid_details','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=800,height=600');
				myWin.moveTo((screen.availWidth/2 - 800/2), (screen.availHeight/2 - 600/2));
				myWin.focus();
			}
