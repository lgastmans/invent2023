<?
	...
	/*
		CUSTOM FIELDS
	*/
	$_SESSION['int_stock_selected'] = 10;
	...
	<head>
		...
		<script language="javascript">
			/*
				CUSTOM FIELDS
			*/
			var SQL = "SELECT * FROM <?php echo Monthalize('dc');?> d LEFT JOIN customer c ON (d.client_id = c.id) ";
			var ID = "dc_id";
			var uniqueFilter = "d.storeroom_id = <?php echo $_SESSION['int_current_storeroom']?>";
			...
			function editRecord() {
				if (selectedID > 0) {
					myWin = window.open("dc_frameset.php?id="+selectedID+'&record_id='+currentRecordID, 'edit', 'width=1000,height=700,resizable=yes,scrollbars=yes');
					myWin.moveTo((screen.availWidth/2 - 1000/2), (screen.availHeight/2 - 700/2));
					myWin.focus();
				}
				else
					alert("select a dc to edit");
			}
			
			var resolveSuccess = function(o) {
			...
			var resolveFailure = function(o) {
				alert('failed to resolve DC');
			}
			...
			function resolve() {
				if (arrRecordIDs.length>1) {
					alert('Please select only one row');
				}
				else if (selectedID > 0) {
					var sUrl = 'dc_resolve.php';
					var postData = "id="+selectedID+"&record_id="+currentRecordID;
					var request = YAHOO.util.Connect.asyncRequest('POST', sUrl, resolveCallback, postData);
				}
				else
					alert("select a dc to edit");
			}
			...
			function printDC() {
				if (selectedID > 0) {
					myWin = window.open("print_dc.php?id="+selectedID, 'print_dc', 'width=1000,height=700,resizable=yes,scrollbars=yes');
					myWin.moveTo((screen.availWidth/2 - 1000/2), (screen.availHeight/2 - 700/2));
					myWin.focus();
				}
				else
			...
			var handleSuccess = function(o) {
				var r = YAHOO.lang.JSON.parse(o.responseText);
				if (r.dc_status == 1)
					r.dc_status = "Unresolved";
				else if (r.dc_status == 2)
					r.dc_status = "Delivered";
				else if (r.dc_status == 3)
					r.dc_status = "<font color='red'>Cancelled</font>";
				else if (r.dc_status == 4)
					r.dc_status = "<font color='green'>Invoiced</font>";
			...
			function updateRow() {
				...
				var postData = "filter=dc_id|"+selectedID+"&gridname=<?php echo $grid_name;?>&sql="+SQL;
				var request = YAHOO.util.Connect.asyncRequest('POST', sUrl, callback, postData);
			}
			
			function newRecord() {
				myWin = window.open("dc_frameset.php?action=clear_dc",'new_dc','toolbar=no,fullscreen=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=1000,height=700,top=0,left=0');
				myWin.moveTo((screen.availWidth/2 - 1000/2), (screen.availHeight/2 - 700/2));
				myWin.focus();
			}
	...
	<tr>
		<td><?
			if ($int_access_level > ACCESS_READ) {
				...
				echo "&nbsp;<a href=\"javascript:resolve();\"><img src=\"../../images/tick.png\" border=\"0\" title=\"set DC status to delivered\"></a>\n";
				echo "&nbsp;<a href=\"#\" id=\"lnk-invoice\"><img src=\"../../images/table.png\" border=\"0\" title=\"export to invoice\"></a>\n";
			}
		?>
			&nbsp;
			<a href="javascript:printDC();"><img src="../../images/printer.png" border="0" title="print the selected delivery chalan"></a>
			&nbsp;
		...
		<td align="center" class="normaltext">
			...
			<select id="select_status" name="select_status" onchange="javascript:refreshPage()">
				<option value="_ALL" <?php if ($filter_status=='_ALL') echo "selected";?>>All</option>
				<option value="_DELIVERED" <?php if ($filter_status=='_DELIVERED') echo "selected";?>>Delivered</option>
			</select>
			&nbsp;
  ...
  <div class="modal-dialog modal-xl" role="document">
    ...
    <div class="modal-content">
      ...
      <div class="modal-header">
        ...
        <span class="model-title">Create</span>
...
<script>
	...
	$(document).ready(function() {
		...
		$(" #lnk-invoice ").on("click", function(e) {

			e.preventDefault();

			var num = arrRecordIDs.length;

			if (num > 0) {

				if (confirm('are you sure?')) {

					var data = JSON.stringify(arrRecordIDs);
...
//		myBody.className = myBody.className + ' ' + browserType();
		...
		myGrid = function() {
			...
			function getFilters() {
				...
				if (oSelectStatus.value == '_ALL')
					strRetVal = '';
				else
					strRetVal = 'dc_status,2|';
			...
			parseStatus = function(oData) {
				switch (oData) {
					...
					case '2': return 'Delivered';break;
					case '3': return '<font color="red">Cancelled</font>';break;
					case '4': return '<font color="green">Invoiced</font>';break;
			...
			var onCellClick = function(oArgs) {
				...
				*/
				switch (column.action) {
					case 'delete':
						if (canDelete) {
							if (confirm('Are you sure you want to cancel this DC?')) {
								
								YAHOO.util.Connect.asyncRequest(
									'GET',
									'cancel_dc.php?action=delete&id='+selectedID,
									{
										success: function (o) {
											var r = YAHOO.lang.JSON.parse(o.responseText);
											if (r.replyStatus.toUpperCase() == 'OK') {
			...
			var onCellDblClick = function(oArgs) {
				...
				myWin = window.open("dc_edit.php?id="+selectedID, 'edit', 'width=1000,height=700,resizable=yes');
				myWin.moveTo((screen.availWidth/2 - 1000/2), (screen.availHeight/2 - 700/2));
				myWin.focus();
			}
			...
			myDataTable.set("selectionMode","multiple");
			...
			myDataTable.subscribe('rowClickEvent',function(ev) {
				...
				var pos;
				arrRecordIDs.find(function checkID(ID, index) {
				...
				if (pos == null) {
					arrRecordIDs.push(selectedID);
					console.log('selectedID'+selectedID);
				}
				else {
					arrRecordIDs.splice(pos,1);
				}

//				arrRecordIDs.forEach(listIDs);
			});

			// function listIDs(value, index) {
