	...
	*/
	...
	$arr_fields =
		array (
			...
			3 => array (
				'field' => 'account_type',
				'yui_field' => 'account_type',
				'formatter' => 'string',
				'is_custom_formatter' => 'Y',
				'parser' => 'getAccountType',
				'filter' => 'Y',
				'is_primary_key' => 'N',
				'alias' => 'ac'
			),
	...
	<head>
		...
		<script language="javascript">
			/*
				CUSTOM FIELDS
			*/
			var SQL = "SELECT * FROM account_cc ac";
			var ID = "cc_id";
...
//		myBody.className = myBody.className + ' ' + browserType();
		...
		myGrid = function() {
			...
			getAccountType = function(oData) {
				switch (oData) {
					case '3': return "KIND";break;
					case '4': return "CASH";break;
			...
			var onCellDblClick = function(oArgs) {
				...
				myWin = window.open("accounts_grid_details.php?action=view&id="+selectedID ,'tax_grid_details','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=800,height=600');
