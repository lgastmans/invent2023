				...
				if ($result_search->FieldByName('is_available') == 'N')
					...
					for ($i=0; $i<count($_SESSION['order_arr_items']); $i++) {
						if ($_SESSION['order_arr_items'][$i][0] == $strProductCode) {
							...
							$price_found = $_SESSION['order_arr_items'][$int_found][4];
							break;
						}
					}
					// if found, remove before proceeding
					if (($int_found > -1) && ($_SESSION['order_arr_items'][$i][5] == 0)) {
					...
					*/
					if ($price_found == 0) {
						$price_found = getSellingPrice($result_search->FieldByName('product_id'));
					}

					$strDescription = $result_search->FieldByName('product_description')."|". $result_search->FieldByName('measurement_unit')."|".$result_search->FieldByName('is_decimal')."|".$int_found."|".$qty_found."|".$price_found;
				}
			}
		}
