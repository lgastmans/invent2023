		...
		if ($strProductCode != 'nil') {
			...
			if ($result_search->RowCount() > 0) {
				if ($result_search->FieldByName('is_available') == 'N')
					...
					$flt_price = getSellingPrice($result_search->FieldByName('product_id'));
					
					$strDescription = $result_search->FieldByName('product_description')."|". $result_search->FieldByName('measurement_unit')."|".$result_search->FieldByName('is_decimal')."|".$int_found."|".$qty_found."|".$flt_price;
				}
			}
		}
