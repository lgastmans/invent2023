<?
	...
	*/
    ...
    $str_header = str_replace('$bill_number', $qry_bill->FieldByName('bill_number'), $str_header);
    ...
    for ($i=0; $i<$qry_items->RowCount(); $i++) {
        ...
        if ($calculate_tax == 'Y') {
            if ($qry_items->FieldByName('discount') > 0) {
                $discount_price = $flt_price * (1 - ($qry_items->FieldByName('discount')/100));
                $tax_amount = calculateTax($discount_price, $qry_items->FieldByName('tax_id'));
//                $flt_price = $qry_items->FieldByName('price'); //$discount_price + $tax_amount;
                $item_total = $total_quantity * $flt_price; //($total_quantity * ($discount_price + $tax_amount));
            }
            else {
                $tax_amount = calculateTax($flt_price, $qry_items->FieldByName('tax_id'));
...
//                $flt_price = $qry_items->FieldByName('price'); //$discount_price;
                $item_total = $total_quantity * $flt_price; //($total_quantity * $discount_price);
            }
            else {
