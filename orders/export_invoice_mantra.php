...
//echo $sql_items;die();
    ...
    */
    ...
    $qry_settings = new Query("
            SELECT bill_invoice_suffix, bill_pt_bill_number
            FROM user_settings
            WHERE (storeroom_id = ".$_SESSION["int_current_storeroom"].")"
    );
    $suffix = $qry_settings->FieldByName('bill_invoice_suffix');
    
    $filename = 'INVOICE '.sprintf('%03d', $qry_bill->FieldByName('bill_number'))." MANTRA.pdf";

    $invoice_number = "WT/".sprintf('%03d', $qry_bill->FieldByName('bill_number'))."/".$suffix."-MantraB";
...
class PDF extends FPDF
{
	...
	*/
    ...
    $pdf->Cell(20,10,$invoice_number,0,0,'L');
    $pdf->Cell(40);
	...
	$col2 = 145;
    $pdf->Cell($col2,10,'P.O. Number:',0,0,'R');
    $pdf->Cell(20,10,$qry_order->FieldByName('order_reference'),0,0,'L');
    $pdf->Ln(4);
	...
	*/
    ...
    $pdf->SetFont('Arial','',10);
    $pdf->Cell(45,10,'P.O. Date:',0,0,'R');
    $pdf->Cell(20,10,get_date($qry_order->FieldByName('order_date')),0,0,'L');
    $pdf->Ln(4);
...
//    $pdf->Cell(LEN_DISCOUNT,10,'Discount',1,0,'C');
    ...
    */
    ...
    for ($i=0; $i<$qry_items->RowCount(); $i++) {
        ...
        // else
        	$flt_price = $qry_items->FieldByName('mrp');
...
//echo $int_tax_id;
	    ...
	    $pdf->Cell(LEN_DESCRIPTION,$h,substr($qry_items->FieldByName('product_description'),0,30),1,0,'L');
	    $pdf->Cell(LEN_HSN,$h,$qry_items->FieldByName('hsn'),1,0,'C');
	    $pdf->Cell(LEN_QTY,$h,number_format($total_quantity, 0, '.', ''),1,0,'C');
	    $pdf->Cell(LEN_PRICE,$h,number_format($flt_price, 3, '.', ''),1,0,'C');
...
//				echo $tax_amount."::";
    ...
    $flt_total_due = number_format($flt_total_due,2,'.','');

	$exp = explode('.', $flt_total_due);
    $f = new NumberFormatter("en_IN", NumberFormatter::SPELLOUT);
    $str_amount = $f->format($exp[0]). " Paise ". $f->format($exp[1])." only.";
...
//		$pdf->Cell(LEN_DISCOUNT,5,'',1,0,'R');
		...
		$start = L_MARGIN + LEN_SN + LEN_CODE + LEN_DESCRIPTION + LEN_HSN + LEN_QTY + LEN_PRICE + LEN_VALUE;
	}
