...
*/
    ...
    if ($qry_bill->FieldByName('bill_number') > 0)
	    $str = "
	        SELECT 
	    ...
	    $str = "
	        SELECT 
	        	sp.product_code, sp.product_description, sp.tax_id,
	        	bi.price, 
				sc.hsn, 
				smu.measurement_unit,
	        	SUM(bi.quantity + bi.adjusted_quantity) AS quantity
			...
			LEFT JOIN stock_measurement_unit smu ON (smu.measurement_unit_id = sp.measurement_unit_id)
	        WHERE bi.bill_id = $int_id
	        GROUP BY bi.product_id, bi.price
	        ORDER BY sp.product_code
	    ";
	...
	*/
    ...
    for ($i=0; $i<$qry_items->RowCount(); $i++) {
        ...
        if ($qry_customer->FieldByName('is_other_state')=='N') {
			...
			$str_items .= 
				"<tr style='font-family:Arial,sans-serif;font-size:12px;font-weight:normal;'>".
					"<td width='75px'>".$qry_items->FieldByName('product_code')."</td>".
					"<td width='50%'>".$qry_items->FieldByName('product_description')."</td>".
					"<td width='75px'>".$qry_items->FieldByName('hsn')."</td>".
					"<td width='50px' align='right'>".number_format($total_quantity, 0, '.', '')."</td>".
					"<td width='75px' align='right'>".number_format($flt_price, 2, '.', '')."</td>".
					"<td width='50px' align='right'>".$qry_customer->FieldByName('discount')."%</td>".
					"<td width='75px' align='right'>".number_format(($flt_discount_price * $total_quantity), 2, '.', ',')."</td>".
					"<td width='200px' align='right'>".
						"<table>".
							"<tr style='font-family:Arial,sans-serif;font-size:12px;font-weight:normal;'>".
								"<td align='right'>".number_format($tax_rate, 2, '.', '')."%</td>".
...
<table width='100%' border='0'>
	<tr>
		<td align='center' colspan='2'>
			<font style='font-family:Arial,sans-serif;font-size:18px;font-weight:bold;'>TAX INVOICE</br>
			</font></br></br>
		</td>
	</tr>
	<tr>
	...
	<?php if ($qry_customer->FieldByName('is_other_state')=='Y') { ?>
		<td width='100px'>
			<table width='100%' >
				...
				<tr style='font-family:Arial,sans-serif;font-size:12px;font-weight:bold;'><td>Rate</td><td>Amount</td></tr>
			</table>
		</td>
	<?php } else { ?>
...
<tr style='font-family:Arial,sans-serif;font-size:12px;font-weight:normal;'>
	...
	<td align='right'><?php echo number_format($flt_discount, 2, '.', ','); ?></td>
</tr>
