<?
  ...
  require_once "../include/grid.inc.php";
	...
	function getOrderedQty($f_field, $f_qry) {
		echo sprintf("%01.3f", $f_qry->FieldByName($f_field));
	}

	function getQty($f_field, $f_qry) {
		echo sprintf("%01.3f", $f_qry->FieldByName($f_field)+$f_qry->FieldByName('adjusted_quantity'));
	}

$grid = new Grid();
$grid->addColumn("Code", "product_code", "string", true, 50);
$grid->addColumn("Description", "product_description", "string", true, 100);
$grid->addColumn("Ordered", "quantity_ordered", "custom", true, 50, 'getOrderedQty');
$grid->addColumn("Delivered", "quantity", "custom", true, 50, 'getQty');
