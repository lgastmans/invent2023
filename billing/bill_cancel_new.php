...
function cancelBill($f_record_id) {
	...
	if ($qry_bill->RowCount() > 0) {
		...
		// if it is an FS Account bill
		if ($qry_bill->FieldByName('payment_type') == BILL_ACCOUNT) {

			$qry_transfer = new Query("SELECT * FROM ".monthalize('account_transfers')." 
					WHERE  module_record_id = ".$qry_bill->FieldByName('bill_id'));

				if ($qry_transfer->RowCount() > 0) {
					if ($qry_transfer->FieldByName('transfer_status') < 5) {
						$qry_transfer->Query("UPDATE ".monthalize('account_transfers')." 
							SET transfer_status = ".ACCOUNT_TRANSFER_CANCELLED." 
							WHERE module_record_id = ".$qry_bill->FieldByName('bill_id'));
					}
					else {
						echo "<script language=\"javascript\">";
						echo "alert('The transfer for this bill was already completed.')";
						echo "</script>";
					}
				}
		}
		// if it is a PT Account bill
		else if ($qry_bill->FieldByName('payment_type') == BILL_PT_ACCOUNT) {
