...
<script language="javascript">
	...
	<?php if ($_SESSION['current_bill_type'] == BILL_ACCOUNT) { ?>
		var intDiscount = <?php echo $int_fs_account_discount; ?>;
	<?php } else { ?>
		var intDiscount = 0;
	<?php } ?>
...
function stateHandler4() {
	if (requester4.readyState == 4) {
	    if (requester4.status == 200)  {
			...
			//alert(".."+str_retval+".."+oTextBoxQty.value);
...
//====================================================
// get the weight on the connected scale
//====================================================
function getWeight(ProductCode){
	$(document).ready(function() {
		//start the ajax
		$.ajax({
			url: "../common/call_curl.php",
			type: "GET",
			data: "action=weight&product="+ProductCode+"&ip=<?php echo $_SERVER['REMOTE_ADDR'];?>",
			cache: false,
			success: function (html) {
				if (html!=false) {
					document.getElementById('qty').value=html;
				} 
			}
		});
	});
...
function clearValues() {
	...
	oSpan.innerHTML = '&nbsp;';
	<?php if ($_SESSION['current_bill_type'] == BILL_ACCOUNT) { ?>
		intDiscount = <?php echo $int_fs_account_discount; ?>;
	<?php } else { ?>
		intDiscount = 0;
	<?php } ?>
	flt_total_batch_quantity = 0;
...
<form name="billing_enter" method="GET" onsubmit="return false">
	...
	<table width="600" height="30" border="0" cellpadding="0" cellspacing="5">
		...
		<tr>
			...
			<!-- QTY -->
			<? if ($str_edit_price == 'Y') { ?>
				<td width='150px'><input type="text" id="qty" name="qty" value="1" class="<?echo $str_class_input?>" autocomplete="OFF" onkeypress="return focusNext(this, 'price', event)" onfocus="setBatchQty()"></td>
			<? } else { ?>
				<td><input type="text" id="qty" name="qty" value="1" class="<?echo $str_class_input?>" autocomplete="OFF" onkeypress="return focusNext(this, 'code', event)" onfocus="setBatchQty()"></td>
			<? } ?>
