<?
	...
	/*
		THE CATEGORY MONTHLY SALES REGISTER WILL NOT BE CORRECT
		IF PRODUCTS HAVE BEEN DELETED BECAUSE OF THE CATEGORY_ID LINK

		THESE ARE THE EVENTUAL MISSING CATEGORIES 
		DUE TO DELETED PRODUCTS
	*/
	$sql_missing = new Query("
		SELECT *
		FROM ".Monthalize('bill_items')." bi 
		WHERE bi.product_id NOT IN(SELECT product_id FROM stock_product)
	");
	$missing_total = 0;
	$missing_tax = 0;
	if ($sql_missing->RowCount()>0) {

		for ($i=0; $i<$sql_missing->RowCount(); $i++) {

			$missing_total += $sql_missing->FieldByName('price') * ($sql_missing->FieldByName('quantity')+$sql_missing->FieldByName('adjusted_quantity'));
			$missing_tax += $sql_missing->FieldByName('tax_amount');
...
//		$array_header[] = $arr_tmp;
		...
		$array_header[] = array(0=>"H",1=>"HSN ",2=>0,3=>0,4=>COLUMN_ANY); //$arr_tmp;
...
//	$qry_bills = new Query("SELECT * FROM ".Monthalize('bill_items'));
	...
	*/
	...
	for ($cur_day=0; $cur_day<$qry_categories->RowCount(); $cur_day++) {
		...
		$arr_taxes[$cur_day][1] = number_format(intval($qry_categories->FieldByName('hsn')),0,'','');
...
//			}
	...
	$total_rows = $qry_categories->RowCount();

	if ($sql_missing->RowCount()>0) {

		$arr_taxes[$cur_day][0] = "UNKNOWN";
		$arr_taxes[$cur_day][1] = "UNKNOWN";

		$int_col_num = getColumn($array_header, "T");
		$arr_taxes[$cur_day][$int_col_num] = $missing_tax;

		$int_col_num = getColumn($array_header, "S");
		$arr_taxes[$cur_day][$int_col_num] = $missing_total;

		$int_col_num = getColumn($array_header, "GT");
		$arr_taxes[$cur_day][$int_col_num] = $missing_total + $missing_tax;

		$total_rows++;
	}


	for ($cur_day=0; $cur_day<$total_rows;$cur_day++) {
