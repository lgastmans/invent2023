...
function openSearch() {
	myWin = window.open("../../common/product_search.php?formname=billing_enter&fieldname=code",'searchProduct','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=400,height=600,top=0');
...
function checkQty(aValue) {
	...
	if (can_bill == true) {
		...
		if (str_edit_price == 'Y')
			...
			strPassPrice = arr_batch_prices[0];
...
function focusNext(aField, focusElem, evt) {
	...
	else if (charCode == 115) { // F4 Discount
