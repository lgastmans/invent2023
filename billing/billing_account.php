...
<script language="javascript">
	...
	function stateHandler() {
		...
		oStatus = document.getElementById('account_status');
				...
				if (arrRetVal[0] == 'OK') {
					if (strPTAccountsEnabled == 'Y')
						...
						$.ajax({
							...
							data: { fsAccount: oTextBoxNumber.value }
						})
						.done(function( msg ) {
							arr = $.parseJSON( msg );
							if (arr['Result']=='OK') {
								...
								if ((amt > 0) || ((amt == -1))) {
									oStatus.innerHTML = 'Balance available';
									$("#account_status").css("color", "green");
								}
								else {
									oStatus.innerHTML = 'Insufficient Balance';
									$("#account_status").css("color", "red");
								}
							}
							else {
								oStatus.innerHTML = 'Error Retrieving Balance';
								$("#account_status").css("color", "orange");
							}
						});						
...
<table border="0" cellpadding='0' cellspacing='2'>
	...
	<tr>
		<td>
			<input type="text" class="<?echo $str_class_input?>" name="account_number" <?if ($_SESSION['bill_id'] > -1) echo "readonly";?> value="<?echo $_SESSION['current_account_number']?>" onkeypress="focusNext(this, 'code', event, <? echo $_SESSION['current_bill_type']?>)" autocomplete="OFF">
		</td>
		<td>
