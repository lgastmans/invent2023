...
//    require_once("../Numbers/Words.php");
    ...
    //ALTER TABLE `company` ADD `footer` VARCHAR(512) NOT NULL AFTER `email`;
...
//print_r($arr_taxes);
	...
	$qry_customer = new Query("
        SELECT *
        FROM customer
        WHERE id = ".$qry_bill->FieldByName('customer_id')
    );
	...
	*/
	...
	if ($qry_customer->FieldByName('same_address')=='Y') {
		...
		$ship_state = $qry_customer->FieldByName('state');
		...
		$ship_state = $qry_customer->FieldByName('ship_state');
...
class PDF extends FPDF
{
	...
	function Header()
	{
	    ...
	    // Title
    	$this->Cell(30,10,'TAX INVOICE',0,0,'C');
	    $this->Ln(5);
	    ...
	    //$this->Cell(80);
	    $this->Cell($halfWidth,10,$this->company->FieldByName('trust'),0,0,'L');
	    $this->Cell(90,10,$this->company->FieldByName('legal_name'),0,0,'L');
	    $this->Ln(5);

	    $this->Cell($halfWidth,10,$this->company->FieldByName('gstin'),0,0,'L');
	    $this->Cell(90,10,$this->company->FieldByName('address'),0,0,'L');
	    $this->Ln(5);

	    $this->Cell($halfWidth,10,$this->company->FieldByName('email'),0,0,'L');
	    $this->Cell(90,10,$this->company->FieldByName('address2'),0,0,'L');
	    $this->Ln(5);
...
*/
	...
	function Footer()
	{
	    ...
	    // Page number
	    //$this->Cell(0,10,'Bankers: ICICI, Auroville Branch. Account Name: Work Tree, Account No: 163105000220, IFSC: ICIC0001631',0,0,'C');
	...
	*/
	...
	$pdf->Cell(20,10,'Invoice:',$border,0,'L');
	$pdf->Cell(($halfWidth-20),10,$qry_bill->FieldByName('bill_number').$suffix,$border,0,'L');
	$pdf->Cell(20,10,'Date:',$border,0,'L');
	$pdf->Cell(($halfWidth-40),10,get_date($qry_bill->FieldByName('date_created')),$border,0,'L');
	$pdf->Cell(40);
	/*
	$pdf->Cell(30,10,'Date & Time of Supply:',0,0,'R');
	if (date('Y',$qry_bill->FieldByName('supply_date_time')) != '1970') {
	...
	*/
	...
	elseif ($qry_bill->FieldByName('payment_type') == BILL_ACCOUNT) {
		$filename_customer = $qry_bill->FieldByName('account_number')." - ".$qry_bill->FieldByName('account_name');
	    $pdf->SetFont('Arial','',10);
	    $pdf->Cell(55,10,'FS Account: '.$qry_bill->FieldByName('account_number')." - ".$qry_bill->FieldByName('account_name'),0,0,'L');
	    $pdf->Ln(4);
	}
	elseif ($qry_bill->FieldByName('payment_type') == BILL_AUROCARD) {
		$filename_customer = 'Aurocard: '.$qry_bill->FieldByName('aurocard_number');
	    $pdf->SetFont('Arial','',10);
	    $pdf->Cell(55,10,'Aurocard: '.$qry_bill->FieldByName('aurocard_number')." Trans Id: ".$qry_bill->FieldByName('aurocard_transaction_id'),0,0,'L');
	    $pdf->Ln(4);
	}
	elseif ($qry_bill->FieldByName('payment_type') == BILL_CREDIT_CARD) {
		$filename_customer = 'Credit Card: '.$qry_bill->FieldByName('payment_type_number');
	    $pdf->SetFont('Arial','',10);
	    $pdf->Cell(55,10,'Credit Card: '.$qry_bill->FieldByName('payment_type_number'),0,0,'L');
	    $pdf->Ln(4);
	}
    ...
    */
    ...
    $pdf->Cell(LEN_PRICE, 10,'Price',1,0,'C');
    $pdf->Cell(LEN_DISCOUNT,10,'Discount',1,0,'C');
    $y=$pdf->GetY();
    $x=$pdf->GetX();
    ...
    */
    ...
    for ($i=0; $i<$qry_items->RowCount(); $i++) {
        ...
        if ($is_taxed == 'Y') {
            ...
            else if ($qry_bill->FieldByName('payment_type') == BILL_ACCOUNT) {
	    ...
	    // code
		$result = $qry_items->FieldByName('product_code');
		if (str_word_count($result)>1) {
			$s = substr($result, 0, LEN_CODE);
			$result = substr($s, 0, strrpos($s, ' '));
		}
	    $pdf->Cell(LEN_CODE,$h,$result,1,0,'R');
	    // description
	    $result = $qry_items->FieldByName('product_description');
	    if (str_word_count($result)>1) {
			$s = substr($result, 0, LEN_DESCRIPTION-10 );
	    	$result = substr($s, 0, strrpos($s, ' '));
	    }
	    $pdf->Cell(LEN_DESCRIPTION,$h,$result,1,0,'L');
	    ...
	    $y=$pdf->GetY();
	    $x=$pdf->GetX();
	    $pdf->MultiCell(LEN_VALUE,$h,number_format(($flt_discount_price * $total_quantity),2,'.',','),1,'R');
	    $pdf->SetXY($x+15,$y);
	...
	*/
	...
	$flt_grand_total = $flt_total + $flt_handling + $flt_handling_tax - $flt_discount;
    $str_amount = ExpandAmount(number_format($flt_grand_total,2,'.',''));
    ...
    */
	if ($qry_customer->FieldByName('is_other_state')=='Y') {
		...
		$start = L_MARGIN + LEN_SN + LEN_CODE + LEN_DESCRIPTION + LEN_HSN;
		$pdf->SetX($start);
		$pdf->Cell(LEN_QTY,5,number_format(($qty_total)),1,0,'C');
		$pdf->Cell(LEN_PRICE,5,'',1,0,'R');
		$pdf->Cell(LEN_DISCOUNT,5,'',1,0,'R');
	    $pdf->Cell(LEN_VALUE,5,number_format(($flt_total-$tax_total),2),1,0,'R');
	...
	if ($qry_customer->FieldByName('is_other_state')=='Y')
		$start = L_MARGIN + LEN_SN + LEN_CODE + LEN_DESCRIPTION + LEN_HSN + LEN_QTY + LEN_PRICE;
	else
	...
	*/
	...
	$pdf->Ln(15);
	$pdf->SetX(142);
	$pdf->Cell(30,5,'for '.$company->FieldByName('legal_name'),0,0,'R');
	$pdf->Ln(5);

	$title = 'Invoice';
	$pdf->SetTitle($title);
	$pdf->SetAuthor($company->FieldByName('legal_name'));

    $filename = 'Invoice '.$qry_bill->FieldByName('bill_number').$suffix." - ".$filename_customer.".pdf";
