	...
	*/
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded'){
		...
		if($type_ns != '' && $type_ns == $this->namespaces['xsd']){
			...
			$type_prefix = 'ns1';//.rand(1000,9999);  # akash
					...
					if(is_array($val) && count($val)> 0){
						...
						if(count($array_types) > 1){
							...
							if ($tt_ns != '' && $tt_ns == $this->namespaces['xsd']){
								...
								$tt_prefix = 'ns1';// . rand(1000, 9999); #akash
...
//		}
		...
		if (
			...
			$this->setError("Unsupported HTTP response status $http_status $http_reason (mysoapclient->response has contents of the response)");
			return false;
		}
...
* @access   public
*/
class mysoapclient extends nusoap_base  {
	...
	* @access   public
	*/
	function mysoapclient($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 300, $response_timeout = 3000){
		parent::nusoap_base();
	...
	*/
	function getProxy(){
		...
		$r = rand();
		$evalStr = 'class soap_proxy_'.$r.' extends mysoapclient {
				'.$evalStr.'
			}';
