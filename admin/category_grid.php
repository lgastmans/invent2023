<?
	...
	/*
		CUSTOM FIELDS
	*/
	$_SESSION['int_admin_selected'] = 2;
	
	$grid_name = 'admin_category';
	$default_alias = 'cat';
...
/*			
		...
		<script language="javascript">
			/*
				CUSTOM FIELDS
			*/
			var SQL = "SELECT cat.category_id, cat.category_code, cat.hsn, cat.category_description, cat.is_perishable, pcat.category_description AS `parent_category` FROM stock_category cat LEFT JOIN stock_category pcat ON (pcat.category_id = cat.parent_category_id)";
			var ID = "category_id";
			...
			function editRecord() {
				if (selectedID > 0) {
					myWin = window.open("category.php?id="+selectedID, 'edit', 'width=800,height=700,resizable=yes');
			...
			function newRecord() {
				myWin = window.open("category.php",'insert','toolbar=no,fullscreen=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=800,height=700,top=0,left=0');
				myWin.moveTo((screen.availWidth/2 - 800/2), (screen.availHeight/2 - 700/2));
				myWin.focus();
			}
	...
	<tr>
		<td><?
			if ($int_access_level > ACCESS_READ) {
				echo "&nbsp;<a href=\"javascript:newRecord();\"><img src=\"../images/page_white_add.png\" border=\"0\" title=\"add a new measurement unit\"></a>\n";
				echo "&nbsp;<a href=\"javascript:editRecord();\"><img src=\"../images/page_white_edit.png\" border=\"0\" title=\"edit the details of the selected measurement unit\"></a>\n";
			}
		?>
			&nbsp;
						...
						if (canDelete) {
							if (confirm('Are you sure you want to delete this row?')) {
								...
								YAHOO.util.Connect.asyncRequest(
									'GET',
									'categorydelete.php?action=delete&id='+selectedID,
									{
										success: function (o) {
