...
/*			.ff th.yui-dt-hidden,
		...
		<script language="javascript">
			/*
				CUSTOM FIELDS
			*/
			var SQL = "SELECT * FROM stock_measurement_unit m";
			var ID = "measurement_unit_id";
			...
			function editRecord() {
				if (selectedID > 0) {
					myWin = window.open("measurement_unit.php?id="+selectedID, 'edit', 'width=800,height=700,resizable=yes');
			...
			function newRecord() {
				myWin = window.open("measurement_unit.php",'insert','toolbar=no,fullscreen=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=800,height=700,top=0,left=0');
				myWin.moveTo((screen.availWidth/2 - 800/2), (screen.availHeight/2 - 700/2));
				myWin.focus();
			}
						...
						if (canDelete) {
							if (confirm('Are you sure you want to delete this row?')) {
								...
								YAHOO.util.Connect.asyncRequest(
									'GET',
									'measurementdelete.php?action=delete&id='+selectedID,
									{
										success: function (o) {
