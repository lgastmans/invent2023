<?
	...
	/*
		CUSTOM FIELDS
	*/
	$_SESSION['int_admin_selected'] = 6;
	
	$grid_name = 'admin_storeroom';
	$default_alias = 'st';
	...
	$arr_fields =
		array (
			...
			1 => array (
				'field' => 'storeroom_code',
				'yui_field' => 'storeroom_code',
				'formatter' => 'string',
				'is_custom_formatter' => 'N',
				'parser' => 'string',
				'filter' => 'Y',
			...
			4 => array (
				'field' => 'bill_credit_account',
				'yui_field' => 'bill_credit_account',
				'formatter' => 'string',
				'is_custom_formatter' => 'N',
				'parser' => 'string',
				'filter' => 'Y',
			...
			5 => array (
				'field' => 'is_account_taxed',
				'yui_field' => 'is_account_taxed',
				'formatter' => 'boolean',
				'is_custom_formatter' => 'N',
				'parser' => 'boolean',
				'filter' => 'N',
			...
			8 => array (
				'field' => 'can_bill_pt_account',
				'yui_field' => 'can_bill_pt_account',
				'formatter' => 'boolean',
				'is_custom_formatter' => 'N',
				'parser' => 'boolean',
				'filter' => 'N',
			...
			9 => array (
				'field' => 'can_bill_fs_account',
				'yui_field' => 'can_bill_fs_account',
				'formatter' => 'boolean',
				'is_custom_formatter' => 'N',
				'parser' => 'boolean',
				'filter' => 'N',
...
/*			.ff th.yui-dt-hidden,
		...
		<script language="javascript">
			/*
				CUSTOM FIELDS
			*/
			var SQL = "SELECT st.*, ss.supplier_name, tax.tax_description FROM stock_storeroom st LEFT JOIN  stock_supplier ss ON (st.default_supplier_id = ss.supplier_id) LEFT JOIN <?php echo Monthalize('stock_tax')?> tax ON (st.default_tax_id = tax.tax_id)";
			var ID = "storeroom_id";
			...
			function editRecord() {
				if (selectedID > 0) {
					myWin = window.open("storeroom_edit.php?id="+selectedID, 'edit', 'width=800,height=700,resizable=yes');
			...
			function newRecord() {
				myWin = window.open("storeroom_edit.php",'insert','toolbar=no,fullscreen=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=800,height=700,top=0,left=0');
				myWin.moveTo((screen.availWidth/2 - 800/2), (screen.availHeight/2 - 700/2));
				myWin.focus();
			}
	...
	<tr>
		<td><?
			if ($int_access_level > ACCESS_READ) {
				echo "&nbsp;<a href=\"javascript:newRecord();\"><img src=\"../images/page_white_add.png\" border=\"0\" title=\"add a new storeroom\"></a>\n";
				echo "&nbsp;<a href=\"javascript:editRecord();\"><img src=\"../images/page_white_edit.png\" border=\"0\" title=\"edit the details of the selected storeroom\"></a>\n";
			}
		?>
			&nbsp;
...
/*		var myBody = document.getElementById('myBody');
		...
		myGrid = function() {
			...
			YAHOO.widget.DataTable.Formatter.formatImage = function(el, oRecord, oColumn, oData) {
				if (YAHOO.lang.isString(oData)) {
					...
					if (YAHOO.lang.isString(eImage) && eImage.length) {
						el.innerHTML = "<img src='../images/products/" + eImage + "'>";
					} else {
						el.innerHTML = oData;
					}
				} else {
						...
						if (canDelete) {
							if (confirm('Are you sure you want to delete this row?')) {
								...
								YAHOO.util.Connect.asyncRequest(
									'GET',
									'storeroomdelete.php?action=delete&id='+selectedID,
									{
										success: function (o) {
			...
			var onCellDblClick = function(oArgs) {
				...
				myWin = window.open("storeroom_edit.php?action=view&id="+selectedID ,'tax_grid_details','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=650,height=550');
