...
/*			.ff th.yui-dt-hidden,
		...
		<script language="javascript">
			...
			*/
			...
			function editRecord() {
				if (selectedID > 0) {
					myWin = window.open("user_edit.php?id="+selectedID, 'edit', 'width=800,height=700,resizable=yes');
			...
			function newRecord() {
				myWin = window.open("user_edit.php",'insert','toolbar=no,fullscreen=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=800,height=700,top=0,left=0');
				myWin.moveTo((screen.availWidth/2 - 800/2), (screen.availHeight/2 - 700/2));
				myWin.focus();
			}
			...
			function changePassword() {
				if (selectedID > 0) {
					myWin = window.open("user_password.php?id="+selectedID,'user','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=450,height=250,top=0');
	...
	<tr>
		<td><?
			if ($int_access_level > ACCESS_READ) {
				if ($int_access_level==ACCESS_ADMIN) {
					echo "&nbsp;<a href=\"javascript:newRecord();\"><img src=\"../images/user_add.png\" border=\"0\" title=\"add a new user\"></a>\n";
				}
				echo "&nbsp;<a href=\"javascript:editRecord();\"><img src=\"../images/user_edit.png\" border=\"0\" title=\"edit the details of the selected user\"></a>\n";
			}
			if ($int_access_level==ACCESS_ADMIN) 
				echo "&nbsp;<a href=\"javascript:changePassword();\"><img src=\"../images/textfield_key.png\" border=\"0\" title=\"Change password\"></a>\n";
		?>
			&nbsp;
...
/*		var myBody = document.getElementById('myBody');
		...
		myGrid = function() {
			...
			getUserType = function(oData) {
				switch (oData) {
					case '1': return 'Normal User';break;
						...
						if (canDelete) {
							if (confirm('Are you sure you want to delete this row?')) {
								...
								YAHOO.util.Connect.asyncRequest(
									'GET',
									'userdelete.php?action=delete&id='+selectedID,
									{
										success: function (o) {
