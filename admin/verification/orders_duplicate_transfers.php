	...
	*/
	$qry_duplicates = new Query("
		...
		WHERE transfer_id NOT IN ( 
			SELECT transfer_id
			FROM ".Monthalize('account_transfers')." 
			GROUP BY account_from, module_record_id, module_id
			HAVING ( COUNT(*) = 1)
		)
		AND module_id = 7
		ORDER BY account_from, transfer_id, description
	");
...
<?php
	...
	if ($qry_duplicates->RowCount() > 0) {
		...
		for ($i=0;$i<$qry_duplicates->RowCount();$i++){
			echo "<tr>";
				echo "<td>account ".$qry_duplicates->FieldByName("account_from")."</td><td>".
					$qry_duplicates->FieldByName('description')."</td><td>".
					$qry_duplicates->FieldByName('amount')."</td><td>".
					$qry_duplicates->FieldBYName('module_record_id')."</td><td>".
					get_transfer_status($qry_duplicates->FieldByName('transfer_status'))."</td>";
			echo "</tr>";
			$qry_duplicates->next();
		}
		echo "</table>";
...
?>
	<br><br>
	<input type='button' name='action' value='Remove duplicate transfers' onclick='setPending()'>
	<input type='hidden' name='action' value='mark_pending'>

<?php
	}
	else {
		echo "There are no duplicate transfers.";
	}
?>
