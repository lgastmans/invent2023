...
$grid = new DBGrid('admin_communities');

$grid->addColumn("ID", "c.community_id", "string", true, 50); 
$grid->addColumn("Community", "c.community_name", "string", true); 

$grid->loadView();

$grid->setQuery("SELECT
  c.community_id AS `c.community_id`,
...
");
  ...
  $grid->setSubmitURL('communities.php');
...
function newRecord() {
       myWin = window.open("community_new.php",'community_new','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=450,height=200,top=0');
...
function modifyRecord() {
  sNo = getSelectedSerialNumber();
  if (sNo>0) {
       myWin = window.open("community_edit.php?id="+intSerialNumber,'account_edit','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=450,height=200,top=0');
       myWin.moveTo((screen.availWidth/2 - 450/2), (screen.availHeight/2 - 200/2));
       myWin.focus();
  }
  else {
...
if ($int_access_level > ACCESS_READ) {
  $grid_form->addButton('Add new community','../images/new.gif','newRecord','left');	
	$grid_form->addButton('Edit community','../images/modify.gif','modifyRecord','left');	
  $grid_form->addButton('Delete selected community','../images/delete.gif','deleteRecord','left');	
}
...
$grid_form->setFrames('communitiesmenu','communitiescontent');
$grid_form->draw();
