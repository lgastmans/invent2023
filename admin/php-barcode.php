...
//$font_loc2=dirname($_SERVER["PATH_TRANSLATED"])."/"."arialn.ttf";
putenv('GDFONTPATH=' . realpath('.'));

$font_loc="arialbd.ttf";
$font_loc2="ARIALN.TTF";//arialn.ttf";
...
if (isset($_ENV['windir']) && file_exists($_ENV['windir'])) {
    ...
    $font_loc2=$_ENV['windir']."\Fonts\arialn.ttf";
}
*/
...
//    $total_y = 300;
    if (!$space)
      $space = array('top'=>2*$scale+10,'bottom'=>2*$scale,'left'=>2*$scale,'right'=>2*$scale);
    ...
    // write the info text, if any
    if (!empty($info)) {
        imagettftext($im, 8, 0, 5, 10, $col_text, $font_loc2, $info);
    }
	...
	header("Content-Type: image/jpg; name=\"barcode.jpg\"");
	imagejpeg($im); //, "e:/xampp/tmp/barcode.jpg", 100);
    } else if ($mode=='gif'){
...
function barcode_print($code, $encoding="ANY", $scale = 4 ,$mode = "png", $info_string ){
    ...
    else barcode_outimage($bars['text'],$bars['bars'],$scale, $mode,50,'',$info_string);
    return $bars;
}
