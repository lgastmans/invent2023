...
$grid = new DBGrid('books_languages');

$grid->addColumn("language", "language", "string", true, 150);
$grid->addColumn("group", "language_group", "string", true);
...
$grid->setOnClick('gridClick','language_id');
$grid->setSubmitURL('languages.php');
$grid->processParameters($_GET);

if (!empty($_GET["action"]))
	if ($_GET["action"]=="del") {
		require("language_delete.php");
	
		$_SESSION['str_message'] = deleteLanguage($_GET["delid"]);
	
		header("Location:languages.php?".$grid->buildQueryString());
		exit;
	}
...
<script language='javascript'>
	...
	function deleteRecord() {
		...
		if (sNo>0) {
			...
			alert("Select a language to delete");
	}
	
	function newRecord() {
		myWin = window.open("language_edit.php",'currency','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=450,height=200,top=0');
	...
	function modifyRecord() {
		sNo = getSelectedSerialNumber();
		if (sNo>0) {
			myWin = window.open("language_edit.php?id="+intSerialNumber,'currency','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=450,height=200,top=0');
			myWin.moveTo((screen.availWidth/2 - 450/2), (screen.availHeight/2 - 200/2));
			myWin.focus();
		}
		else {
			alert("Select a language to modify!");
		}
	}
</script>
</head>
...
if ($int_access_level > ACCESS_READ) {
	$grid_form->addButton('Add new language','../images/new.gif','newRecord','left');
	$grid_form->addButton('Edit language','../images/modify.gif','modifyRecord','left');
	$grid_form->addButton('Delete selected language','../images/delete.gif','deleteRecord','left');
}
...
$grid_form->setFrames('language_menu','language_content');
$grid_form->draw();
