<?php
	...
	/*
		CUSTOM FIELDS
	*/
	$_SESSION["int_books_menu_selected"] = 2;

	$grid_name = 'books_publishers';
	$default_alias = 'sp';
...
/*			.ff th.yui-dt-hidden,
		...
		<script language="javascript">
			/*
				CUSTOM FIELDS
			*/
			var SQL = "SELECT * FROM stock_publisher sp ";
			var ID = "publisher_id";
			...
			function editRecord() {
				if (selectedID > 0) {
					myWin = window.open("publisher_edit.php?id="+selectedID, 'edit', 'width=800,height=700,resizable=yes');
					myWin.moveTo((screen.availWidth/2 - 800/2), (screen.availHeight/2 - 700/2));
					myWin.focus();
				}
				else
					alert('select a language to edit');
			}
			
			function newRecord() {
				myWin = window.open("publisher_edit.php",'insert','toolbar=no,fullscreen=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,width=800,height=700,top=0,left=0');
				myWin.moveTo((screen.availWidth/2 - 800/2), (screen.availHeight/2 - 700/2));
				myWin.focus();
			}
	...
	<tr>
		<td><?
			if ($int_access_level > ACCESS_READ) {
				echo "&nbsp;<a href=\"javascript:newRecord();\"><img src=\"../images/page_white_add.png\" border=\"0\" title=\"add a new language\"></a>\n";
				echo "&nbsp;<a href=\"javascript:editRecord();\"><img src=\"../images/page_white_edit.png\" border=\"0\" title=\"edit the details of the selected language\"></a>\n";
			}
		?>
			&nbsp;
						...
						if (canDelete) {
							if (confirm('Are you sure you want to delete this row?')) {
								...
								YAHOO.util.Connect.asyncRequest(
									'GET',
									'publisher_delete.php?action=delete&id='+selectedID,
									{
										success: function (o) {
