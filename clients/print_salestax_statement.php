...
<html>
    <head>
        <style type='text/css'>
<!--
        .po_header {font-family:Verdana,sans-serif;font-size:10px;font-weight:bold;color: black;}
        .po_text {font-family:Verdana,sans-serif;font-size:10px;font-weight:normal;color: black;}
-->
        </style>
    </head>
<body>

<font style='font-family:Verdana;font-size:14px;font-weight:normal;'>
<?
echo "<b>".$str_application_title."</b><br>";
echo $str_user_address."<br>";
echo $str_user_phone."<br><br>";
?>
Sales tax statement for <?echo getMonthName($_SESSION['int_month_loaded'])." - ".$_SESSION['int_year_loaded'];?>
</font>
...
<table border='1' cellpadding='5' cellspacing='0'>
	...
	<?
		for ($i=0; $i<count($array_header); $i++) {
			echo "<td align='center' valign='center' class='po_header' width='80px'><b>".$array_header[$i][1]."</b></td>";
		}
	?>
	</tr>
	<?
		...
		for ($i=0;$i<count($array_content);$i++) {
			...
			echo "<td align='right' class='po_text'>".$array_content[$i][0]."</td>";
			echo "<td class='po_text'>".$array_content[$i][1]."</td>";
			echo "<td class='po_text'>".$array_content[$i][2]."</td>";
			echo "<td align='right' class='po_text'>".number_format($array_content[$i][3],2,'.','')."</td>";
			$total += number_format($array_content[$i][3],2,'.','');
			for ($j=4;$j<count($array_header);$j++) {
				if (IsSet($array_content[$i][$j])) {
					echo "<td align='right' class='po_text'>".number_format($array_content[$i][$j],2,'.',',')."</td>";
    ...
    ?>
	<tr bgcolor='white'>
		<td align='right' colspan='3' class='po_header'>Totals</td>
		<td align='right' class='po_header'><?echo number_format($total,2,'.',',');?></td>
		<?
			foreach ($arr_totals as $value)
				echo "<td align='right' class='po_header'>".number_format($value,2,'.',',')."</td>";
		?>
	</tr>
</table>
